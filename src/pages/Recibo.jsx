  ...
  useEffect(() => {
    async function carregar() {
      ...
      const [lancRes, osRes] = await Promise.all([
        supabase
          .from('lancamentos')
          .select('*, clientes(nome, telefone, documento, endereco), fornecedores(nome, telefone, documento, endereco)')
          .eq('id', id)
          .single(),
  ...
  return (
    <div className="max-w-xl mx-auto py-6 px-4 print:p-0 print:max-w-full">
      <div className="flex items-center justify-between mb-6 no-print">
        <Link to="/contas-a-receber" className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
      ...
      <div className="mb-4 no-print">
        <label className="block text-xs text-gray-500 mb-1">Garantia (dias, opcional — aparece no recibo)</label>
        <input
          type="number"
          value={garantiaDias}
      ...
      <div className="bg-white border border-gray-200 print:border-0 rounded-lg p-8 print:p-0 print:rounded-none">
        <div className="flex items-center justify-between border-b-2 border-gray-800 pb-4 mb-6">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-gray-400">
              Emitido em {formatDateBR(todayISO())}
            </p>
          </div>
        </div>
        ...
        <p className="text-sm text-gray-500 mb-12">Para maior clareza firmamos o presente recibo.</p>
